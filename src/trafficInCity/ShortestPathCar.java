...
public class ShortestPathCar extends Car {
	...
	public void move() {
		...
		if(a) {
			Car c = ContextManager.carContext.getRandomObject();
			AID receiver = (AID) c.getAID();
			sendMessage(receiver, "oi");
		}
		
		ACLMessage msg = receive();
		if (msg!=null && a) {
			if(msg.getContent().equals("oi") ) {
				AID carAID = msg.getSender();
				System.out.println(carAID + ": " + msg.getContent());
				sendMessage(carAID, "oi para ti também");
			}
			else if(msg.getContent().equals("oi para ti também")) {
				AID carAID = msg.getSender();
				System.out.println(carAID + ": " + msg.getContent());
				a = false;
			}
